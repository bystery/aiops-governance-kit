...
- M1 核心部分：Node 标准库安装器、写入前预检与失败回退、可见 `guanjia/` 资料夹、原子写入、受管 `AGENTS.md` 区块、资源 manifest、分级 doctor/strict 诊断和旧 `.aiops/` 保留提示。
- M2 核心部分：`state.json`、revision 乐观并发检查、本地锁、checkpoint、带版本/现场摘要的 handoff、过期交接检测、resume、状态面板和固定交接模板。
- M4 基础部分：任务范围/完成标准/复用字段、风险级别与验证模式、契约修订失效旧证据、结构化验证证据、任务契约摘要、argv 执行、超时/命令缺失的 NOT_RUN、低风险文档轻量快照验证、快照绑定、暂存区检查。
...
- M5 增量：`.aiops` 迁移 dry-run、显式 apply、冲突识别、源目录保留，以及带摘要预检、显式 apply、备份和保守回退的卸载事务。
...
- 完整的 `.aiops` 语义迁移仍需按旧项目实际规则补充映射；卸载核心事务已落地，但状态、证据、交接历史和迁移归档仍按设计保留，不会静默清空。提交 hook 已支持受保护的 `hooks uninstall` 回退，但不会自动删除状态、证据或覆盖用户修改。

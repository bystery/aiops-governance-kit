...
- M4 基础部分：任务范围/完成标准/复用字段、结构化验证证据、任务契约摘要、argv 执行、超时/命令缺失的 NOT_RUN、快照绑定、暂存区检查。
- M0 回归：已有规则不覆盖、特殊字符项目名、重复安装、用户暂停、坏状态/缺资源等核心 fixture。
- M3/M4 增量：ZCode/Codex 适配契约、可审查的 ZCode marketplace/Codex hooks 模板、Node 薄桥接脚本、精简宿主事件与会话 ID 记录、唯一 nonce 宿主探针、真实回执记录、组合式 pre-commit 安装和 doctor 生效判断。

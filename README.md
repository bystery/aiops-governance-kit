...
这套框架用"制衡"而不是"自觉"来治它们：两轮实战会话逐轮审计出 **19 条机制缺陷 + 10 条执行失误 + 3 条使用问题**，全部转化为条文；**25 起历史事故全部编号入账**，其中 22 起在终验中逐一回放，验证"新条文能拦住当初那次翻车"——对照节选见[证据附录](docs/EVIDENCE.md)。
...
- **25 起历史事故**全部编号入账，22 起"事故 × 拦截条文"反事实验证全数通过——抽样对照见[证据附录](docs/EVIDENCE.md)；

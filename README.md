...
- 跨平台安装：字面处理中文、空格、`A/B`、`R&D` 等项目名；写入前预检受管资源、状态 JSON 和 `AGENTS.md` 区块，安装可重复执行且失败不留下半套资料；原有 `AGENTS.md` 受管区块之外的内容保留。
...
- 任务契约：开始任务必须登记范围、完成标准、风险级别和复用判断；低风险纯文案可走受限轻量快照验证，代码/迁移/状态任务仍需明确命令验证。
